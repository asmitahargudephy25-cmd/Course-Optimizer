...
for s in semesters_list:
    semester_credits = sum(x[(c,s)] * int(credits_dict[c]) for c in courses_list)
    robust_model.Add(semester_credits <= max_credits)
    robust_model.Add(semester_credits >= min_credits)
...
robust_model.Minimize(penalty_workload + penalty_timimgs + penalty_gaps + imbalance + penalty_stability)
